...
public class MapEx {
    ...
    *
    * 특징
    * 키는 중복 허용하지 않음 : 동일한 키로 여러 번 데이터를 저장하면, 기존의 값이 새로운 값ㅇ로 덮어씌워짐
    * 값은 중복을 허용        : 동일한 값은 다른 키에 여러 번 저장될 수 있음
    * 순서                    : HashMap 순서를 보장하지 않지만
    *                          LinkeHashMap 입력 순서 유지
    *                           TreeMap   키의 자연 순서 or 지정된 순서로 정렬
    ...
    containKey(Object key) : 지정한 키가 존재하는지 확인
    ...
    */
    public static void main(String[] args) {
        ...
        map.put("사과", 1500);
        System.out.println("사과의 가격이 변경되었습니다: " + map.get("사과") + "원");

        // 바나나 판매합니까? Yes or No
        if (map.containsKey("바나나")) {
            System.out.println("바나나 판매합니까? Yes");
        } else {
            System.out.println("바나나 판매합니까? No");
        }

        // 3000원 짜리 과일이 존재합니까? System 출력문 이용해서 확인 Yes or No
        if (map.containsValue(3000)) {
            System.out.println("3000원 짜리 과일이 존재합니까? Yes");
        } else {
            System.out.println("3000원 짜리 과일이 존재합니까? No");
        }
        ...
        // 모든 키와 값 출력
        System.out.println("모든 키와 값: " + map);
    }
}
